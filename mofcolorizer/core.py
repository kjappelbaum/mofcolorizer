...
SCALER = joblib.load(
    os.path.join(THIS_DIR, "scaler_run_2020_07_21_08_50_1595314233.joblib")
)
MODEL_MEDIAN = joblib.load(
    os.path.join(THIS_DIR, "regressor_medianrun_2020_07_21_08_50_1595314233.joblib")
)
MODEL_01 = joblib.load(
    os.path.join(THIS_DIR, "regressor_0_1run_2020_07_21_08_50_1595314233.joblib")
)
MODEL_09 = joblib.load(
    os.path.join(THIS_DIR, "regressor_0_9run_2020_07_21_08_50_1595314233.joblib")
)
...
def predict(cif):
    ...
    if not isinstance(features, pd.DataFrame):  # pylint:disable=no-else-return
        ...
        hex_median = [
            rgb_to_hex((int(c[0]), int(c[1]), int(c[2]))) for c in prediction_median
        ][0]
        hex_01 = [rgb_to_hex((int(c[0]), int(c[1]), int(c[2]))) for c in prediction_01][
            0
        ]
        hex_09 = [rgb_to_hex((int(c[0]), int(c[1]), int(c[2]))) for c in prediction_09][
            0
        ]
        return html.Div(
            [
